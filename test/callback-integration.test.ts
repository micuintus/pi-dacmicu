...
// 10. Non-bash tools are not touched
{
	const { pi, fireToolCall, shutdown } = createMockPi();
	callbackFactory(pi);
...
// 11. CLI binary: happy path — receives prompt, prints text, exits 0
{
...
// 12. CLI binary: server reports ok:false → exit 1, error to stderr
{
...
// 13. CLI binary: missing $PI_CALLBACK_SOCKET → exit 2, helpful error
{
...
// 14. CLI binary: missing prompt → exit 2, usage help
{
...
// 15. CLI binary: connecting to nonexistent socket → exit 1, socket error on stderr
{
...
// 16. CLI binary: malformed reply line → exit 1, parse error
{
	const server = await startTestServer((_line, write, end) => {
...
// 17. CLI binary: multi-word prompt joined with spaces (sanity for shell args)
{
	let received = "";
	const server = await startTestServer((line, write, end) => {
